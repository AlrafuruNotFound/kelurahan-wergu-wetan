...
4. **[`Root Gateways`](./.cursorrules)** — Jika Anda menggunakan AI IDE (Cursor, Windsurf), mereka akan otomatis membaca `.cursorrules` di root.
...
### 3. Code & Commit
Gunakan standar **Conventional Commits** yang sering dipakai oleh profesional untuk memudahkan *tracking* perubahan. Format: `<type>(<scope>): <subject>`

**Daftar Type (Jenis Commit):**
- 🌟 `feat:` — Menambah fitur baru
- 🐛 `fix:` — Memperbaiki bug/error
- 📝 `docs:` — Mengubah dokumentasi (`README`, `.docs/`)
- 🎨 `style:` — Perubahan formatting, spasi, CSS statis (tidak mengubah logika)
- ♻️ `refactor:` — Menulis ulang/merapikan kode (bukan fitur baru, bukan bug fix)
- ⚡ `perf:` — Peningkatan performa aplikasi (optimasi query, render)
- 🧪 `test:` — Menambah/memperbaiki pengujian (testing)
- 🔧 `chore:` — Update dependency, config (package.json, gitignore)
- 👷 `ci:` — Perubahan alur CI/CD (GitHub Actions)
...
- ✅ Panggil `verifySession()` atau `verifySuperAdmin()` di dalam `try/catch` di awal Server Action administratif untuk mengamankan eksekusi tingkat server
