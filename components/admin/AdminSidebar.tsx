...
export default function AdminSidebar({ role, isCollapsed = false }: { role: "admin" | "super", isCollapsed?: boolean }) {
  ...
  return (
    <SidebarContext.Provider value={{ isCollapsed }}>
      <aside className={`fixed top-0 left-0 z-40 flex h-screen flex-col justify-between border-r border-slate-800 bg-slate-900 text-white transition-all duration-300 ${isCollapsed ? "w-20" : "w-64"}`}>
        <div className="flex h-full flex-col overflow-hidden">
          {/* HEADER SIDEBAR */}
          <div className={`shrink-0 ${isCollapsed ? "p-4 flex justify-center" : "p-6"}`}>
            <div className="mb-4 flex items-center gap-3">
              <div
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl shadow-lg transition-all duration-300 ${logoBgClass}`}
              >
                {isSuper ? (
                  <ShieldAlert size={22} />
                ) : (
              ...
              {!isCollapsed && (
                <div className="transition-opacity duration-300">
                  <h1 className="text-lg leading-none font-bold tracking-tight text-slate-200 truncate w-32">
                    Wergu Admin
                  </h1>
                  <span
        ...
        <div className="no-scrollbar flex-1 space-y-1.5 overflow-y-auto px-4 pb-6">
          ...
          {isSuper && (
            <div className="animate-in fade-in slide-in-from-left-4 mt-6 border-t border-white/5 pt-4 duration-500">
              ...
              <Link
                href="/admin/settings"
                title={isCollapsed ? "Manajemen Admin" : ""}
                className={`group relative flex items-center ${isCollapsed ? "justify-center px-2 py-3" : "gap-3 px-4 py-3"} overflow-hidden rounded-xl text-sm font-medium transition-all ${
              ...
              >
                ...
                />
                {!isCollapsed && <span className="relative z-10 truncate">Manajemen Admin</span>}
      ...
      {/* FOOTER SIDEBAR */}
      <div className={`shrink-0 border-t border-white/5 bg-slate-900/50 ${isCollapsed ? "p-3" : "p-4"}`}>
        <Link
          href="/admin/login"
          onClick={handleLogout}
          title={isCollapsed ? "Keluar Sistem" : ""}
          className={`group flex items-center ${isCollapsed ? "justify-center px-2 py-3" : "gap-3 px-4 py-3"} rounded-xl text-sm font-medium text-slate-400 transition-all hover:bg-white/5 hover:text-white`}
        >
          <LogOut
            size={isCollapsed ? 20 : 18}
...
function SidebarDropdown({ label, icon: Icon, isActive, accent, children }: any) {
  ...
  return (
    <div className="flex flex-col gap-1" title={isCollapsed ? label : ""}>
      <button
        onClick={() => !isCollapsed && setIsOpen(!isOpen)}
        className={`group flex w-full items-center ${isCollapsed ? "justify-center px-2 py-3" : "justify-between px-4 py-3"} rounded-xl text-sm font-medium transition-all duration-300 ${isActive || (isOpen && !isCollapsed) ? activeClass : "text-slate-400 hover:bg-slate-800"}`}
      >
        <div className="flex items-center gap-3">
          <Icon
            size={20}
            className={`shrink-0 transition-transform duration-300 ${isActive || (isOpen && !isCollapsed) ? iconClass : ""}`}
          />
...
function SidebarLink({ href, icon: Icon, label, isActive, accent, isSub = false }: any) {
  ...
  return (
    <Link
      ...
      className={`group relative flex items-center ${isCollapsed ? "justify-center px-2 py-3" : "gap-3 px-4 py-3"} overflow-hidden rounded-xl font-medium transition-all duration-300 ${isSub ? "text-[13px]" : "text-sm"} ${isActive ? activeClass : `text-slate-400 ${hoverClass}`}`}
    >
      <Icon
        size={isSub ? 16 : 20}
