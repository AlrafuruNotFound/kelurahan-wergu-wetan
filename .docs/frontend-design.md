...
## 🖼️ 8. Image Handling & Core Web Vitals

Untuk menjaga skor **Lighthouse (LCP)** dan optimasi otomatis ke format **WebP**:
- **WAJIB** menggunakan komponen `<Image>` dari `next/image` sebagai ganti tag HTML standar `<img>` atau *CSS backgroundImage*.
- Untuk gambar *responsive* atau *cover/hero*, gunakan atribut `fill` dikombinasikan dengan `sizes` yang tepat untuk menghindari pengunduhan gambar besar di layar _mobile_.
- Gunakan atribut `priority={true}` KHUSUS untuk gambar utama yang muncul di *above the fold* (hero/banner pertama).

```typescript
// ❌ DILARANG (Memicu LCP lambat, file size besar)
<div style={{ backgroundImage: `url('/images/hero.png')` }} />
<img src="/images/hero.png" className="w-full h-full object-cover" />

// ✅ WAJIB (Otomatis WebP, ukuran disesuaikan perangkat)
...
**Catatan untuk Stitch Preview (opsional):**
Jika menggunakan Stitch, pastikan `src` menggunakan **absolute URLs**.
...
### Mobile UX & Interaction Rules
Untuk mencegah *bug double-click* yang sering terjadi di browser *mobile* (iOS Safari/Android Chrome) akibat bentrok dengan *pseudo-class* `hover:`, **WAJIB** menambahkan *feedback* sentuhan eksplisit pada elemen interaktif:
- Gunakan `active:scale-95` (atau variasi state `active:` lainnya) pada SEMUA tombol, tautan navigasi, dan elemen *clickable* (seperti tab atau kartu berita). Hal ini memberikan respons visual instan bagi pengguna *mobile*.

---

## ♿ 10. Accessibility (A11y) Rules

Portal desa melayani seluruh lapisan masyarakat, sehingga standar aksesibilitas W3C wajib diterapkan:
- **Button tanpa teks (hanya ikon):** WAJIB memiliki atribut `aria-label` (contoh: `<button aria-label="Halaman Berikutnya"><ChevronRight/></button>`).
- **Contrast Ratio:** Teks sekunder/muted (`text-text-muted`) dan elemen navigasi (seperti *tabs*) harus memiliki kontras rasio minimal `3:1` terhadap latar belakang.
- **Link Sosial/Eksternal:** WAJIB diberikan `aria-label` jika *anchor tag* hanya membungkus ikon tanpa ada label teks.
- **Heading Order:** Urutan judul harus hierarkis (`<h1>` → `<h2>` → `<h3>`), jangan melompat langsung dari `<h1>` ke `<h3>`.

---

## 🧪 11. Before PR: FE Checklist
...
- [ ] Accessibility: keyboard nav, contrast ratio ≥4.5:1, `aria-label` pada icon-only buttons
- [ ] Performance: `<img>` & `backgroundImage` sudah di-*replace* dengan `<Image>` dari `next/image`
