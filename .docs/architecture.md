...
**Created by:** [AlrafuruNotFound](https://github.com/AlrafuruNotFound) | **Version:** 3.9.0  
**Last Updated:** 20 Mei 2026 | **Status:** ✅ Verified Against Codebase v1.0.0
...
- **Authentication:** `jose` (JWT HS256) dengan HTTP-only cookies dan modul verifikasi sesi terpusat (`lib/safe-action.ts`).
...
- 🌐 **Local Dev Network Law (Strict HTTP):** Dilarang keras menggunakan `--experimental-https` atau *self-signed certificates* lokal. Ini memicu HSTS block di Mobile Browser yang menghancurkan rendering JavaScript. Pengujian lokal wajib menggunakan standar `http://` (misal: `next dev -H 0.0.0.0`).
- 🧹 **Orphaned Files Purge Law:** AI wajib mencari, menganalisis, dan memusnahkan file/folder sisa (contoh: `certificates/` lama) yang tidak lagi relevan dengan arsitektur saat ini agar proyek tetap bersih dan tidak menimbulkan kebingungan.
...
| `lib/db.ts`, `lib/safe-action.ts`, `lib/services/*` | BE | `be/*` |
...
| `ulasan.action.ts` | `UlasanLayanan` | Reviews & IKM |
...
6. 🛑 **NEVER** run ANY `git` operations (`git add`, `git commit`, `git checkout`, `git merge`, `git push`) automatically. You MUST set the command to require explicit user confirmation (e.g., SafeToAutoRun=false) so the user gets an "allow" pop-up to analyze the changes first.

**Prompt template (Jika Root Gateway Gagal Dibaca):**
```
Follow ANF-Agentic Architecture:
- WAJIB baca file root `.cursorrules` atau `.docs/architecture.md` sebelum memulai.
...
| 3.7.0 | 2026-05-16 | Backend Fitur Ulasan & IKM (hapus tabel usang PengajuanSurat) | Antigravity |
| 3.7.1 | 2026-05-19 | Frontend Performance & A11y: Enforce `next/image` and `aria-label` policies di `frontend-design.md`, apply fixes to `BeritaView.tsx`, `HomeView.tsx`, `StaticBanner.tsx` | Antigravity |
| 3.7.4 | 2026-05-20 | Frontend Stabilization: Penyelarasan penuh `next/image`, `active:scale-95`, dan `aria-label` di seluruh komponen publik (`HomeView`, `TentangKamiView`, `PotensiDesaView`, dll) | Antigravity |
| **3.8.0** | **2026-05-20** | **Vibe Coding & MCP Standardization: Tambah 5 Root Gateway AI (`.cursorrules`, dll), tegakkan Anti Auto-Git secara algoritmik, rombak ulang SOP `mcp-and-skills.md` untuk modularitas tanpa instalasi server fisik** | **Antigravity** |
| **3.9.0** | **2026-05-20** | **Security Hardening: Implementasi central session verification helper (`lib/safe-action.ts`) dan pengerasan seluruh Server Actions administratif** | **Antigravity** |
**Review Cadence:** Every sprint retro or major feature addition.  
**Approval:** Tech Lead sign-off required for any deviation.

---

**Last Review:** 20 Mei 2026 | **Status:** ✅ Verified Against Codebase v0.1.0  
