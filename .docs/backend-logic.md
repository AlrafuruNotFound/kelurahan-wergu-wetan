...
- Skema database aktif berfokus pada model-model berikut:
  - `Admin`: Kredensial masuk administratif dengan peran `admin` atau `super`.
  - `Kegiatan`: Data artikel berita, pengumuman, dan publikasi kelurahan.
  - `PotensiDesa`: Informasi UMKM lokal dan potensi kelurahan.
  - `BannerHomepage`: Koleksi gambar slide banner di halaman utama.
  - `HomeStatistic`: Data statistik kelurahan (kependudukan, RT/RW, dll).
  - `HomeWelcome`: Pesan sambutan resmi Kepala Kelurahan (Lurah).
  - `HomeService` (menggantikan konsep `Layanan` usang): Informasi ikon direktori layanan yang dikelola via `layanan.action.ts`.
  - `PerangkatDesa`: Struktur organisasi kelurahan beserta urutan tampilannya.
  - `ProfilKonten`: Halaman teks statis dinamis (visi, misi, sejarah).
  - `SiteConfig`: Konfigurasi kontak global situs (nomor telepon, email, alamat).
  - `UlasanLayanan` (Baru): Ulasan kepuasan warga dan penilaian indeks kepuasan masyarakat (IKM).
  - `PesanMasuk`: Data pesan/pertanyaan dari warga yang dikirim lewat formulir kontak.
- **Model Terdepresiasi (Deprecated):**
  - `Umkm` & `Penduduk`: Model lama yang sudah tidak digunakan di dalam kode fungsional dan berstatus tidak aktif.
...
> 1. Jika fitur AI memerlukan akses ke database, **DILARANG** menulis query database langsung di luar *backend workspace*. AI (Skills) **wajib** memanggil fungsi Core Services yang ada di `actions/` atau `lib/`.
> 2. 🛑 **GIT SAFETY RULE:** AI Agent **DILARANG KERAS** melakukan operasi eksekusi `git` secara *auto-run*. Selalu picu konfirmasi eksplisit (via parameter `SafeToAutoRun=false`) agar pengguna bisa mengizinkan dan meninjau perubahan terlebih dahulu.
...
export default async function AdminLayout({ children }) {
  ...
  if (pathname !== '/admin/login') {
    ...
    // Jika berada di halaman login tetapi token ada, decode perannya
    try {
      const secretKey = new TextEncoder().encode(process.env.JWT_SECRET || 'sangat-rahasia-sekali');
      const { payload } = await jwtVerify(token, secretKey);
      if (payload.role === 'super') {
...
### Central Session Verification (lib/safe-action.ts)

Untuk mengamankan Server Actions dari akses tidak sah, kita memusatkan logika verifikasi sesi di [safe-action.ts](file:///c:/jsapp/kelurahan-wergu-wetan/lib/safe-action.ts).

Fungsi utama:
- `verifySession()`: Memastikan pemanggil adalah Admin terautentikasi (memiliki cookie `admin_token` JWT yang valid). Melempar *Error* jika tidak sah.
- `verifySuperAdmin()`: Memastikan pemanggil adalah Super Admin terautentikasi (peran `super`). Melempar *Error* jika tidak sah.

**Cara Penggunaan di Server Actions:**
Setiap Server Action administratif wajib memanggil fungsi verifikasi ini di awal eksekusi di dalam blok `try/catch` (atau di awal fungsi jika mengembalikan promise void):
```typescript
...
**Last Review:** 20 Mei 2026 | **Status:** ✅ Active  
