...
> [!TIP]
> **Automated Protection:** Sistem ini sekarang dilindungi secara lokal oleh **Pre-commit Hook Validator** (`scripts/security-validator.js`) yang secara otomatis menggagalkan commit jika mendeteksi adanya kebocoran berkas rahasia (`.env`, `.pem`) atau string sensitif (`postgresql://`, `JWT_SECRET` mentah).
...
### Server Action Auth Guard Pattern (safe-action.ts)

Sebagai tambahan pengamanan di tingkat rute layout admin, seluruh Server Actions yang melakukan mutasi data administratif dilindungi oleh pemeriksaan keamanan di tingkat server:

- Fungsi `verifySession()` dipanggil untuk memastikan cookie JWT admin aktif dan valid.
- Fungsi `verifySuperAdmin()` dipanggil untuk memastikan admin memiliki peran `super` untuk akses sensitif (seperti manajemen pengguna admin).

**Implementasi Keamanan di Server Actions:**
Pemeriksaan dilakukan secara langsung di dalam Server Actions dan wajib dibungkus dalam blok `try/catch` agar tidak menghasilkan unhandled promise rejections ke klien:

```typescript
export async function editData(id: number, formData: FormData) {
  try {
    await verifySession(); // Guard
    // ... logic ...
    return { success: true };
  } catch (error: any) {
...
Sebelum melakukan `git commit` dan melakukan merge PR, pastikan semua pengecekan otomatis berikut lolos:
- **Git Hook Check:** Seluruh file yang di-stage wajib lolos dari pemindaian `scripts/security-validator.js`.
...
## 🤖 11. AI Agent Security Rules (Git Safety & Transparency)

**For AI Agents (Antigravity, Claude Code, etc.):**
- 🛑 **GIT TRANSPARENCY & SAFETY:** AI Agents MUST NOT run any `git` commands silently. Always generate a `git diff` preview in chat and wait for explicit user approval ("Approve"/"Oke") before executing `git commit` or `git push`.
- 🔐 **LOCAL VALIDATION:** AI Agents must ensure the local pre-commit hook validator script (`scripts/security-validator.js`) is active and has successfully scanned staged changes.
